...
try:
    ...
    pasien_df = pasien_df.withColumn(
        "email_masked",
        F.concat(
            F.substring("email", 1, 3),  # Ambil 3 karakter pertama dari email
            F.lit("*********"),         # Tambahkan masking berupa bintang
            F.lit("@"),                # Tambahkan simbol "@"
            F.split("email", "@").getItem(1)  # Ambil domain setelah simbol "@"
        )
    )
    ...
    pendaftaran_pasien_df = pendaftaran_df.join(pasien_df, pendaftaran_df.id_pasien == pasien_df.id_pasien, "left") \
        .select(
            pendaftaran_df["*"], 
            pasien_df["nama_lengkap"].alias("nama_pasien"),  # Alias untuk nama pasien
            pasien_df["umur"].alias("usia"),               # Alias untuk umur menjadi usia
            pasien_df["jenis_kelamin"],
            pasien_df["pekerjaan"],                        # Tambahan kolom pekerjaan
            pasien_df["email_masked"].alias("email"),      # Menggunakan email yang sudah dimasking
            pasien_df["nomor_telepon_masked"].alias("nomor_telepon"),  # Menggunakan nomor telepon yang sudah dimasking
            pasien_df["kecamatan"]                        # Tambahan kolom kecamatan
        )
